...
class ParseRunExecution:
    ...
    def efficiency_calculator(self, data: list) -> dict:
        ...
        for x, y in data.items():
            for sub_list in y:
                ...
                mem_request.append(
                    normalise_values(self, sub_list[1])
                )  # mem request by user
                cpu_usage.append(
                    math.ceil(float(sub_list[4].split("%")[0]) / 100)
                )  # number of cores used = roundup(CPU_percent  / 100) as it is 100 per core
                mem_usage.append(
                    normalise_values(self, sub_list[-1].strip())
                )  # Using peak mem reported by process
    ...
    def condense(self, data: list):
        ...
        efficiency_dict = ParseRunExecution.efficiency_calculator(
            self, data_per_process
        )
        ...
        sorted_dict = dict(
            sorted(condensed.items())
        )  # Alphabetically sort the keys, means all iterations should have same order
    ...
    def condense_data(self, data: dict):
        ...
        for process, data_lists in data.items():
            ...
            if process_entry in ["RAPID", "FULL"]:
                process = ":".join(
                    process.split(":")[3:]
                )  # Gets subworkflows + process inside subworkflows
            elif "SANGERTOL_TREEVAL" in process_entry:  # LEGACY ENTRY POINT
                process = ":".join(
                    process.split(":")[2:]
                )  # Gets subworkflows + process
            else:
                print(
                    "Ooops, I'm programmed to not recognise the given entry point here"
                )
            if len(data_lists) <= 1:
                condensed_data[process] = [
                    int(data_lists[0][0]),
                    normalise_values(self, data_lists[0][1]),
                    normalise_values(self, data_lists[0][1]),
                    ParseRunExecution.calculate_avg_time(
                        fix_time(data_lists[0][3].split(" "))
                    ),
                    int(float(data_lists[0][4].split("%")[0])),
                    int(float(data_lists[0][5].split("%")[0])),
                    round(
                        (
                            normalise_values(self, data_lists[0][6].split("\\")[0])
                            / normalise_values(self, data_lists[0][1])
                        )
                        * 100,
                        0,
                ...
                for i in data_lists:
                    ...
                    memory.append(
                        normalise_values(self, i[1])
                    )  # '130 GB'     - REQUESTED MEM
                    realtime.append(
                        fix_time(
                            i[3].split(" ")
                        )  # '29m 33s'    - REALTIME EXECUTION TIME
                    )
                    cpu_percent.append(
                        int(float(i[4].split("%")[0]))
                    )  # '1441.5%'    - CPU UTILISATION PERCENTAGE
                    mem_percent.append(
                        int(float(i[5].split("%")[0]))
                    )  # '6.5%'       - MEM UTILISATION PERCENTAGE
                    peak_mem.append(
                        normalise_values(self, i[6].split("\\")[0])
                ...
                avg_peak = round(
                    (sum(peak_mem) / len(peak_mem))
                    / (int(sum(memory) / len(memory)))
                    * 100,
                    0,
                )
