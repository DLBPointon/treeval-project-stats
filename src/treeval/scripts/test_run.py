...
tv_counter = 0
tv_counter2 = ( tv_counter + 1 for i in all_data if 'TreeValProject.Summary' in i.data_type)
for i in all_data:
...
if tv_counter == len(all_data):
    ...
    #
    whole_condensed_data = ExecutionCondenser(pl.concat([i.execution.data_frame for i in new_data]))
    ...
    tv_data = False
    condensed_data = ExecutionCondenser(pl.concat([i.execution.data_frame for i in all_data]))
