...
class ParseExecution:
    ...
    def generate_dataframe(self, data: list) -> pl.DataFrame:
        ...
        process_updates =   (
                            cleaned_df
                                .with_columns(
                                    extracted = pl.col('name').str.split_exact(' ', 1).struct.rename_fields(['name1', 'unneeded'])
                                .alias('process_list')
                                )
                            )


        return reorder(process_updates.unnest('extracted').drop(['name', 'unneeded']), 0, 'name1')


    def remove_common_string(self, smallest_process: list, process_list: list, process_count: int) -> list:
        """
        ...
        return [':'.join([ii for ii in i if not ii in remove_list]) for i in process_list] # Nested list comprehension! Doge Wow!
    ...
    def correct_dataframe_names(self, df: pl.DataFrame, new_names: list) -> pl.DataFrame:
        """
    ...
    def add_column_desctiption(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        new_df =    df.with_columns(
                        ...
                        .then('WSSP')
                        .otherwise('TooManyNests')
                        .alias('desc')
                    )
        return reorder(new_df, 1, 'desc')
